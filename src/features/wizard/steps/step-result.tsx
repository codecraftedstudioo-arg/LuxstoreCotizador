    ...
    <Card className="text-center">
      ...
      <div className="sticky top-0 z-10 bg-black -mx-3 px-3 -mt-2 pt-6 pb-4 mb-4 border-b border-white/10 shadow-[0_4px_12px_rgba(0,0,0,0.8)]">
        <p className="text-white/70 text-sm mb-1">
          {lang === 'es' ? `Tu ${state.model} de ${state.storage ? (state.storage === '1024' ? '1 TB' : state.storage + ' GB') : ''} vale` : `Your ${state.model} ${state.storage ? (state.storage === '1024' ? '1 TB' : state.storage + ' GB') : ''} is worth`}
        </p>
      ...
      <div className="mt-6 p-4 bg-white/5 rounded-xl text-left border border-white/10">
        ...
        <div className="space-y-3">
          ...
          <a
            ...
            onClick={(e) => {
              e.preventDefault()
              const link = buildInquiryLink(state, priceResult, { name: contactName || undefined, phone: contactPhone || undefined }, lang, rate)
              window.fbq?.('track', 'Contact')
